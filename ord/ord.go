...
func NewInscriptionToolWithBtcApiClient(net *chaincfg.Params, btcApiClient btcapi.BTCAPIClient, request *InscriptionRequest, witnessSize int) (*InscriptionTool, error) {
...
func NewInscriptionRevealWithBtcApiClient(net *chaincfg.Params, btcApiClient btcapi.BTCAPIClient, request *InscriptionRequest, witnessSize int) (*InscriptionTool, int64, error) {
	tool := &InscriptionTool{
		net: net,
		client: &blockchainClient{
			btcApiClient: btcApiClient,
	...
	totalRevealPrevOutput, err := tool._getRevealPrevOutput(net, request)
	if err != nil {
		return nil, 0, err
	}
	return tool, totalRevealPrevOutput, nil
}

func (tool *InscriptionTool) _getRevealPrevOutput(net *chaincfg.Params, request *InscriptionRequest) (int64, error) {
	...
	return totalRevealPrevOutput, nil
...
func (tool *InscriptionTool) buildCommitTx(commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64) error {
	...
	noWitnessSize := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx)))
	witnessSize := len(tx.TxIn) * tool.WitnessSize
	fee := (noWitnessSize + btcutil.Amount(witnessSize)) * btcutil.Amount(commitFeeRate)
...
func (tool *InscriptionTool) signCommitTx() error {
	if len(tool.commitTxPrivateKeyList) == 0 {
		...
		for i := range tool.commitTx.TxIn {
			...
			//p2wpkh
			signature, err := txscript.RawTxInWitnessSignature(tool.commitTx, txscript.NewTxSigHashes(tool.commitTx, tool.commitTxPrevOutputFetcher), i, txOut.Value, txOut.PkScript, txscript.SigHashAll, tool.commitTxPrivateKeyList[i])
			witness := wire.TxWitness{signature, tool.commitTxPrivateKeyList[i].PubKey().SerializeCompressed()}
			// p2tr
...
func (tool *InscriptionTool) GetCommitTx() *wire.MsgTx {
	return tool.commitTx
}

func (tool *InscriptionTool) GetCommitTxHash() string {
	return tool.commitTx.TxHash().String()
}
...
func (tool *InscriptionTool) GetRevealTxList() ([]*wire.MsgTx, error) {
	return tool.revealTx, nil
}
